...
const banner = `
  _                        _____
 | |  _  _ _ _  __ _ _ _  |_   _|___ __ _ _ _ Comfort Mod
 | |_| || | ' \/ _` + "`" + ` | '_|   | |/ -_)/ _` + "`" + ` | '_|
...
func main() {
	...
	forceHidden := flag.Bool("force-hidden-story-missions", true, "Present hidden-story missions (category 7) to clients as already completed. Default = true; pass --force-hidden-story-missions=false to require clearing them legitimately")
	flag.Parse()

	forceHiddenStoryMissions = *forceHidden
...
func launchDev(ip string, p ports) {
	...
	// cmd/dev defaults the flag to true; only forward the opt-out explicitly.
	if !forceHiddenStoryMissions {
		devArgs = append(devArgs, "--force-hidden-story-missions=false")
	}
