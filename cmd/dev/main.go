...
func main() {
	...
	forceHiddenStoryMissions := flag.Bool("force-hidden-story-missions", true, "Present hidden-story missions (category 7) to clients as already completed. Default = true; pass --force-hidden-story-missions=false to require clearing them legitimately")
	...
	services := []service{
		...
		},
		{
			...
			cmd: exec.CommandContext(ctx, filepath.Join("bin", "lunar-tear"+ext),
				grpcArgs(*grpcListen, *grpcPublicAddr, *grpcDB, *grpcOctoURL, *grpcAuthURL, *adminListen, *noRegister, *forceHiddenStoryMissions)...,
			),
		},
	}
...
func grpcArgs(listen, publicAddr, db, octoURL, authURL, adminListen string, noRegister, forceHiddenStoryMissions bool) []string {
	args := []string{
		"--listen", listen,
	...
	// lunar-tear defaults to true; only forward the opt-out explicitly.
	if !forceHiddenStoryMissions {
		args = append(args, "--force-hidden-story-missions=false")
	}
	return args
}
