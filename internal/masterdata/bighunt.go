...
type BigHuntCatalog struct {
	...
	BossQuestGroupIdByBossId    map[int32]int32 // Maps BigHuntBossId to BigHuntBossQuestGroupId
}
...
// CollectHighestReward returns only the reward of the highest threshold the
// score has reached — the player's current rank reward. Unlike
// CollectNewRewards it never sums the rewards of every lower tier together.
// Thresholds are sorted ascending by NecessaryScore at load time.
func (c *BigHuntCatalog) CollectHighestReward(scoreRewardGroupId int32, score int64) []RewardItem {
	thresholds := c.ScoreRewardThresholds[scoreRewardGroupId]
	var bestGroupId int32
	found := false
	for _, t := range thresholds {
		if score >= t.NecessaryScore {
			bestGroupId = t.BigHuntRewardGroupId
			found = true
		} else {
			break
		}
	...
	return c.RewardItems[bestGroupId]
}
...
func LoadBigHuntCatalog() *BigHuntCatalog {
	...
	// Build mapping from BigHuntBossId to BigHuntBossQuestGroupId
	bossQuestGroupRows, err := utils.ReadTable[EntityMBigHuntBossQuestGroup]("m_big_hunt_boss_quest_group")
	if err != nil {
		log.Fatalf("load big hunt boss quest group table: %v", err)
	}
	bossQuestGroupIdByBossId := make(map[int32]int32)
	for _, r := range bossQuestGroupRows {
		if bossQuest, ok := bossQuestById[r.BigHuntBossQuestId]; ok {
			bossQuestGroupIdByBossId[bossQuest.BigHuntBossId] = r.BigHuntBossQuestGroupId
		}
	}
	...
	return &BigHuntCatalog{
		...
		BossQuestGroupIdByBossId:    bossQuestGroupIdByBossId,
	}
}
