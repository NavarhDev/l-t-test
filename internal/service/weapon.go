...
func (s *WeaponServiceServer) EnhanceByMaterial(ctx context.Context, req *pb.EnhanceByMaterialRequest) (*pb.EnhanceByMaterialResponse, error) {
	...
	_, err := s.users.UpdateUser(userId, func(user *store.UserState) {
		...
		if totalMaterialCount > 0 {
			levelIncrease := weapon.Level - oldLevel
			if levelIncrease > 0 {
				ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 5, TargetId: weapon.WeaponId, Delta: levelIncrease}, nowMillis)
			}
			ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 43, TargetId: weapon.WeaponId, CurrentValue: weapon.Level}, nowMillis)
		}
...
func (s *WeaponServiceServer) Evolve(ctx context.Context, req *pb.EvolveRequest) (*pb.EvolveResponse, error) {
	...
	_, err := s.users.UpdateUser(userId, func(user *store.UserState) {
		...
		ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 7, TargetId: oldWeaponId, Delta: 1}, nowMillis)
...
func (s *WeaponServiceServer) EnhanceSkill(ctx context.Context, req *pb.EnhanceSkillRequest) (*pb.EnhanceSkillResponse, error) {
	...
	_, err := s.users.UpdateUser(userId, func(user *store.UserState) {
		...
		ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 6, TargetId: weapon.WeaponId, Delta: addCount}, nowMillis)
		ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 44, TargetId: req.SkillId, CurrentValue: skills[skillIdx].Level}, nowMillis)
...
func (s *WeaponServiceServer) EnhanceAbility(ctx context.Context, req *pb.EnhanceAbilityRequest) (*pb.EnhanceAbilityResponse, error) {
	...
	_, err := s.users.UpdateUser(userId, func(user *store.UserState) {
		...
		user.WeaponAbilities[req.UserWeaponUuid] = abilities
		ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 45, TargetId: req.AbilityId, CurrentValue: abilities[abilityIdx].Level}, nowMillis)
...
func (s *WeaponServiceServer) LimitBreakByMaterial(ctx context.Context, req *pb.LimitBreakByMaterialRequest) (*pb.LimitBreakByMaterialResponse, error) {
	...
	_, err := s.users.UpdateUser(userId, func(user *store.UserState) {
		...
		user.Weapons[req.UserWeaponUuid] = weapon
		if totalMaterialCount > 0 {
			ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 8, TargetId: weapon.WeaponId, Delta: totalMaterialCount}, nowMillis)
		}
...
func (s *WeaponServiceServer) LimitBreakByWeapon(ctx context.Context, req *pb.LimitBreakByWeaponRequest) (*pb.LimitBreakByWeaponResponse, error) {
	...
	_, err := s.users.UpdateUser(userId, func(user *store.UserState) {
		...
		user.Weapons[req.UserWeaponUuid] = weapon
		if consumedCount > 0 {
			ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 8, TargetId: weapon.WeaponId, Delta: consumedCount}, nowMillis)
		}
...
func (s *WeaponServiceServer) EnhanceByWeapon(ctx context.Context, req *pb.EnhanceByWeaponRequest) (*pb.EnhanceByWeaponResponse, error) {
	...
	_, err := s.users.UpdateUser(userId, func(user *store.UserState) {
		...
		user.Weapons[req.UserWeaponUuid] = weapon
		if consumedCount > 0 {
			ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 5, TargetId: weapon.WeaponId, Delta: 1}, nowMillis)
			ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: 43, TargetId: weapon.WeaponId, CurrentValue: weapon.Level}, nowMillis)
		}
...
func (s *WeaponServiceServer) Awaken(ctx context.Context, req *pb.WeaponAwakenRequest) (*pb.WeaponAwakenResponse, error) {
	...
	_, err := s.users.UpdateUser(userId, func(user *store.UserState) {
		...
		ApplyMissionProgressEvent(user, cat.Mission, MissionProgressEvent{ConditionType: missionConditionWeaponAwaken, TargetId: weapon.WeaponId, Delta: 1}, nowMillis)
	})
	if err != nil {
