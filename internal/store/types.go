...
// QuestSceneChoiceState represents the current selected choice for a grouping.
// key = QuestSceneChoiceGroupingId
...
// QuestSceneChoiceHistoryState represents a choice that was made at some point.
// key = QuestSceneChoiceEffectId
...
type UserState struct {
	...
	MissionPassStates        map[int32]MissionPassPointState
	...
	CostumeLotteryEffectAbilities map[CostumeLotteryEffectKey]UserCostumeLotteryEffectAbilityState
	CostumeLotteryEffectStatusUps map[CostumeLotteryEffectStatusUpKey]UserCostumeLotteryEffectStatusUpState
	...
	QuestSceneChoices             map[int32]QuestSceneChoiceState       // key = QuestSceneChoiceGroupingId
	QuestSceneChoiceHistory       map[int32]QuestSceneChoiceHistoryState // key = QuestSceneChoiceEffectId
	...
	CharacterViewerFields            map[int32]CharacterViewerFieldState          // key: characterViewerFieldId
}
...
func (u *UserState) EnsureMaps() {
	...
	if u.MissionPassStates == nil {
		u.MissionPassStates = make(map[int32]MissionPassPointState)
	}
	...
	if u.CostumeLotteryEffectAbilities == nil {
		u.CostumeLotteryEffectAbilities = make(map[CostumeLotteryEffectKey]UserCostumeLotteryEffectAbilityState)
	}
	if u.CostumeLotteryEffectStatusUps == nil {
		u.CostumeLotteryEffectStatusUps = make(map[CostumeLotteryEffectStatusUpKey]UserCostumeLotteryEffectStatusUpState)
	}
...
type BattleState struct {
	...
	LastCharacterDeathCount     int32
	LastCostumeSkillUsedCount   int32
	LastWeaponSkillUsedCount    int32
	LastCompanionSkillUsedCount int32
	LastCriticalCount           int32
	LastComboCount              int32
	LastComboMaxDamage          int32
	// Number of alive characters at end of last wave
	LastCostumeAliveCount int32
	// Total party size (1-3 characters, determines max possible deaths)
	LastCostumePartySize int32
	// Per-character HP percentage at end of battle (0-100).
	LastCostumeHpPercent [3]int32
	// Total recovery points from recovery skills/abilities
	LastTotalRecoverPoint int64
	// Character IDs in the party during battle (populated from CostumeBattleInfo.DeckCharacterNumber)
	LastPartyCharacterIds [3]int32
	// Main weapon IDs for each character in the party during battle
	LastMainWeaponIds [3]int32
}

// ResetQuestBattleStats clears every per-quest battle statistic that quest
// mission evaluation reads. It is the single reset point, called whenever a
// new quest attempt begins (start or restart) so values never leak between
// attempts or between different quests.
func (b *BattleState) ResetQuestBattleStats() {
	b.LastCharacterDeathCount = 0
	b.LastCostumeSkillUsedCount = 0
	b.LastWeaponSkillUsedCount = 0
	b.LastCompanionSkillUsedCount = 0
	b.LastCriticalCount = 0
	b.LastComboCount = 0
	b.LastComboMaxDamage = 0
	b.LastCostumeAliveCount = 0
	b.LastCostumePartySize = 0
	b.LastCostumeHpPercent = [3]int32{}
	b.LastTotalRecoverPoint = 0
	b.LastPartyCharacterIds = [3]int32{}
	b.LastMainWeaponIds = [3]int32{}
}
...
// MissionPassPointState tracks per-pass point totals and whether the pass
// rewards have already been handed out (ReceiveMissionPassRewards is one-shot).
...
type PartsState struct {
	...
	PartsStatusMainValue int32 // Current value of main stat
	IsProtected          bool
	AcquisitionDatetime  int64
	LatestVersion        int64
}
...
type UserCostumeLotteryEffectAbilityState struct {
	UserCostumeUuid string
	SlotNumber      int32
	AbilityId       int32
	AbilityLevel    int32
...
type UserCostumeLotteryEffectStatusUpState struct {
	UserCostumeUuid       string
	StatusCalculationType int32
	Hp                    int32
	Attack                int32
...
type CostumeLotteryEffectStatusUpKey struct {
	UserCostumeUuid       string
	StatusCalculationType int32
}
...
type CharacterViewerFieldState struct {
	...
	IsNotified            bool   // Было ли показано уведомление клиенту
	NotifiedDatetime      int64
	LatestVersion         int64
}
