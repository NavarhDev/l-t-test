...
func (s *SQLiteStore) UpdateUser(userId int64, mutate func(*store.UserState)) (out store.UserState, err error) {
	defer func() {
		if r := recover(); r != nil {
			log.Printf("[PANIC] UpdateUser user=%d: %v\n%s", userId, r, debug.Stack())
			out = store.UserState{}
			err = fmt.Errorf("panic in UpdateUser: %v", r)
		}
	}()
