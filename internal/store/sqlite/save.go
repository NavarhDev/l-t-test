...
func writeUserState(tx *sql.Tx, uid int64, u *store.UserState) error {
	...
	if err := exec(`INSERT INTO user_battle (user_id, is_active, start_count, finish_count, last_started_at, last_finished_at, last_user_party_count, last_npc_party_count, last_battle_binary_size, last_elapsed_frame_count, last_character_death_count, last_costume_skill_used_count, last_weapon_skill_used_count, last_companion_skill_used_count, last_critical_count, last_combo_count, last_combo_max_damage, last_costume_alive_count, last_costume_party_size, last_costume_hp_percent_0, last_costume_hp_percent_1, last_costume_hp_percent_2, last_total_recover_point, last_party_character_id_0, last_party_character_id_1, last_party_character_id_2, last_main_weapon_id_0, last_main_weapon_id_1, last_main_weapon_id_2) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		...
		u.Battle.LastCharacterDeathCount, u.Battle.LastCostumeSkillUsedCount, u.Battle.LastWeaponSkillUsedCount,
		u.Battle.LastCompanionSkillUsedCount, u.Battle.LastCriticalCount, u.Battle.LastComboCount, u.Battle.LastComboMaxDamage,
		u.Battle.LastCostumeAliveCount, u.Battle.LastCostumePartySize, u.Battle.LastCostumeHpPercent[0], u.Battle.LastCostumeHpPercent[1], u.Battle.LastCostumeHpPercent[2], u.Battle.LastTotalRecoverPoint, u.Battle.LastPartyCharacterIds[0], u.Battle.LastPartyCharacterIds[1], u.Battle.LastPartyCharacterIds[2], u.Battle.LastMainWeaponIds[0], u.Battle.LastMainWeaponIds[1], u.Battle.LastMainWeaponIds[2]); err != nil {
		return err
	}
	...
	for _, v := range u.MissionPassStates {
		if err := exec(`INSERT INTO user_mission_pass_points (user_id, mission_pass_id, point_count, rewards_claimed) VALUES (?,?,?,?)`,
			uid, v.MissionPassId, v.PointCount, boolToInt(v.RewardsClaimed)); err != nil {
			return err
		}
	}
	...
	for key, v := range u.CostumeLotteryEffectAbilities {
		if err := exec(`INSERT OR REPLACE INTO user_costume_lottery_effect_ability 
			(user_id, user_costume_uuid, slot_number, ability_id, ability_level, latest_version) 
			VALUES (?,?,?,?,?,?)`,
			uid, key.UserCostumeUuid, key.SlotNumber, v.AbilityId, v.AbilityLevel, v.LatestVersion); err != nil {
			return err
		}
	}

	for _, v := range u.CostumeLotteryEffectStatusUps {
		if err := exec(`INSERT OR REPLACE INTO user_costume_lottery_effect_status_up 
			(user_id, user_costume_uuid, status_calculation_type, hp, attack, vitality, agility, critical_ratio, critical_attack, latest_version) 
			VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
			uid, v.UserCostumeUuid, v.StatusCalculationType, v.Hp, v.Attack, v.Vitality, v.Agility, v.CriticalRatio, v.CriticalAttack, v.LatestVersion); err != nil {
			return err
		}
	}
	...
	for _, v := range u.CharacterViewerFields {
		if err := exec(`INSERT INTO user_character_viewer_fields (user_id, character_viewer_field_id, is_notified, notified_datetime, latest_version) VALUES (?,?,?,?,?)`,
			uid, v.CharacterViewerFieldId, boolToInt(v.IsNotified), v.NotifiedDatetime, v.LatestVersion); err != nil {
			return err
		}
	}
	...
	if err := exec(`INSERT INTO user_pvp_state (user_id, pvp_point, attack_win_count, attack_lose_count,
		defense_win_count, defense_lose_count, attack_win_streak, last_finish_day, defense_deck_number,
		weekly_reward_version, pending_weekly_reward_group_id, pending_weekly_reward_point,
		pending_weekly_reward_season_id, last_auto_progress_day, matching_refresh_count,
		matching_refresh_day, season_index, latest_version, pvp_deck_backup,
		max_season_rank, max_season_rank_season_id)
		VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		uid, u.Pvp.PvpPoint, u.Pvp.AttackWinCount, u.Pvp.AttackLoseCount, u.Pvp.DefenseWinCount,
		u.Pvp.DefenseLoseCount, u.Pvp.AttackWinStreak, u.Pvp.LastFinishDay, u.Pvp.DefenseDeckNumber,
		u.Pvp.WeeklyRewardVersion, u.Pvp.PendingWeeklyRewardGroupId, u.Pvp.PendingWeeklyRewardPoint,
		u.Pvp.PendingWeeklyRewardSeasonId, u.Pvp.LastAutoProgressDay, u.Pvp.MatchingRefreshCount,
		u.Pvp.MatchingRefreshDay, u.Pvp.SeasonIndex, u.Pvp.LatestVersion, u.Pvp.PvpDeckBackup,
		u.Pvp.MaxSeasonRank, u.Pvp.MaxSeasonRankSeasonId); err != nil {
		return err
	}
...
func diffAndSave(tx *sql.Tx, uid int64, before, after *store.UserState) error {
	...
	if before.Battle != after.Battle {
		if err := exec(`UPDATE user_battle SET is_active=?, start_count=?, finish_count=?, last_started_at=?, last_finished_at=?, last_user_party_count=?, last_npc_party_count=?, last_battle_binary_size=?, last_elapsed_frame_count=?, last_character_death_count=?, last_costume_skill_used_count=?, last_weapon_skill_used_count=?, last_companion_skill_used_count=?, last_critical_count=?, last_combo_count=?, last_combo_max_damage=?, last_costume_alive_count=?, last_costume_party_size=?, last_costume_hp_percent_0=?, last_costume_hp_percent_1=?, last_costume_hp_percent_2=?, last_total_recover_point=?, last_party_character_id_0=?, last_party_character_id_1=?, last_party_character_id_2=?, last_main_weapon_id_0=?, last_main_weapon_id_1=?, last_main_weapon_id_2=? WHERE user_id=?`,
			...
			after.Battle.LastBattleBinarySize, after.Battle.LastElapsedFrameCount,
			after.Battle.LastCharacterDeathCount, after.Battle.LastCostumeSkillUsedCount, after.Battle.LastWeaponSkillUsedCount,
			after.Battle.LastCompanionSkillUsedCount, after.Battle.LastCriticalCount, after.Battle.LastComboCount, after.Battle.LastComboMaxDamage,
			after.Battle.LastCostumeAliveCount, after.Battle.LastCostumePartySize, after.Battle.LastCostumeHpPercent[0], after.Battle.LastCostumeHpPercent[1], after.Battle.LastCostumeHpPercent[2], after.Battle.LastTotalRecoverPoint, after.Battle.LastPartyCharacterIds[0], after.Battle.LastPartyCharacterIds[1], after.Battle.LastPartyCharacterIds[2], after.Battle.LastMainWeaponIds[0], after.Battle.LastMainWeaponIds[1], after.Battle.LastMainWeaponIds[2], uid); err != nil {
			return err
		}
	}
	...
	diffMapInt32(tx, uid, before.MissionPassStates, after.MissionPassStates, "user_mission_pass_points", "mission_pass_id",
		func(v store.MissionPassPointState) []any {
			return []any{v.MissionPassId, v.PointCount, boolToInt(v.RewardsClaimed)}
		}, "mission_pass_id, point_count, rewards_claimed")
	...
	for k, v := range after.CostumeLotteryEffectAbilities {
		if old, ok := before.CostumeLotteryEffectAbilities[k]; !ok || old != v {
			exec(`INSERT OR REPLACE INTO user_costume_lottery_effect_ability (user_id, user_costume_uuid, slot_number, ability_id, ability_level, latest_version) VALUES (?,?,?,?,?,?)`,
				uid, k.UserCostumeUuid, k.SlotNumber, v.AbilityId, v.AbilityLevel, v.LatestVersion)
		}
	}
	for k := range before.CostumeLotteryEffectAbilities {
		if _, ok := after.CostumeLotteryEffectAbilities[k]; !ok {
			exec(`DELETE FROM user_costume_lottery_effect_ability WHERE user_id=? AND user_costume_uuid=? AND slot_number=?`, uid, k.UserCostumeUuid, k.SlotNumber)
		}
	}
	for k, v := range after.CostumeLotteryEffectStatusUps {
		if old, ok := before.CostumeLotteryEffectStatusUps[k]; !ok || old != v {
			exec(`INSERT OR REPLACE INTO user_costume_lottery_effect_status_up (user_id, user_costume_uuid, status_calculation_type, hp, attack, vitality, agility, critical_ratio, critical_attack, latest_version) VALUES (?,?,?,?,?,?,?,?,?,?)`,
				uid, k.UserCostumeUuid, k.StatusCalculationType, v.Hp, v.Attack, v.Vitality, v.Agility, v.CriticalRatio, v.CriticalAttack, v.LatestVersion)
		}
	}
	for k := range before.CostumeLotteryEffectStatusUps {
		if _, ok := after.CostumeLotteryEffectStatusUps[k]; !ok {
			exec(`DELETE FROM user_costume_lottery_effect_status_up WHERE user_id=? AND user_costume_uuid=? AND status_calculation_type=?`, uid, k.UserCostumeUuid, k.StatusCalculationType)
		}
	}
	...
	diffMapInt32(tx, uid, before.CharacterViewerFields, after.CharacterViewerFields, "user_character_viewer_fields", "character_viewer_field_id",
		func(v store.CharacterViewerFieldState) []any {
			return []any{v.CharacterViewerFieldId, boolToInt(v.IsNotified), v.NotifiedDatetime, v.LatestVersion}
		},
		"character_viewer_field_id, is_notified, notified_datetime, latest_version")
	diffMapInt32(tx, uid, before.CageOrnamentAccesses, after.CageOrnamentAccesses, "user_cage_ornament_accesses", "cage_ornament_id",
		...
		"cage_ornament_id, first_access_datetime, latest_access_datetime, latest_version")
	...
	if before.Pvp != after.Pvp {
		exec(`INSERT OR REPLACE INTO user_pvp_state (user_id, pvp_point, attack_win_count, attack_lose_count,
			...
			after.Pvp.DefenseLoseCount, after.Pvp.AttackWinStreak, after.Pvp.LastFinishDay, after.Pvp.DefenseDeckNumber,
			after.Pvp.WeeklyRewardVersion, after.Pvp.PendingWeeklyRewardGroupId, after.Pvp.PendingWeeklyRewardPoint,
			after.Pvp.PendingWeeklyRewardSeasonId, after.Pvp.LastAutoProgressDay, after.Pvp.MatchingRefreshCount,
			after.Pvp.MatchingRefreshDay, after.Pvp.SeasonIndex, after.Pvp.LatestVersion, after.Pvp.PvpDeckBackup,
			after.Pvp.MaxSeasonRank, after.Pvp.MaxSeasonRankSeasonId)
	}
