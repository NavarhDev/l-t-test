...
func initMaps(u *store.UserState) {
	...
	u.CostumeLotteryEffectAbilities = make(map[store.CostumeLotteryEffectKey]store.UserCostumeLotteryEffectAbilityState)
	u.CostumeLotteryEffectStatusUps = make(map[store.CostumeLotteryEffectStatusUpKey]store.UserCostumeLotteryEffectStatusUpState)
...
func load1to1(db *sql.DB, uid int64, u *store.UserState) {
	...
	if err := db.QueryRow(`SELECT is_active, start_count, finish_count, last_started_at, last_finished_at,
		...
		last_companion_skill_used_count, last_critical_count, last_combo_count, last_combo_max_damage,
		last_costume_alive_count, last_costume_party_size, last_costume_hp_percent_0, last_costume_hp_percent_1, last_costume_hp_percent_2, last_total_recover_point, last_party_character_id_0, last_party_character_id_1, last_party_character_id_2, last_main_weapon_id_0, last_main_weapon_id_1, last_main_weapon_id_2
		...
		Scan(&isActive, &u.Battle.StartCount, &u.Battle.FinishCount, &u.Battle.LastStartedAt,
			...
			&u.Battle.LastCostumeAliveCount, &u.Battle.LastCostumePartySize, &u.Battle.LastCostumeHpPercent[0], &u.Battle.LastCostumeHpPercent[1], &u.Battle.LastCostumeHpPercent[2], &u.Battle.LastTotalRecoverPoint, &u.Battle.LastPartyCharacterIds[0], &u.Battle.LastPartyCharacterIds[1], &u.Battle.LastPartyCharacterIds[2], &u.Battle.LastMainWeaponIds[0], &u.Battle.LastMainWeaponIds[1], &u.Battle.LastMainWeaponIds[2]); err != nil && err != sql.ErrNoRows {
		// A scan failure here silently zeroes all battle stats and breaks quest
		// mission evaluation — make it loud instead of swallowing it.
		log.Printf("[LoadUser] user_battle scan failed for user=%d: %v", uid, err)
	}
...
func loadMapTables(db *sql.DB, uid int64, u *store.UserState) {
	...
	queryRows(db, `SELECT mission_pass_id, point_count, rewards_claimed
		FROM user_mission_pass_points WHERE user_id=?`, uid, func(rows *sql.Rows) {
		var v store.MissionPassPointState
		var rc int
		rows.Scan(&v.MissionPassId, &v.PointCount, &rc)
		v.RewardsClaimed = rc != 0
		u.MissionPassStates[v.MissionPassId] = v
	})
	...
	queryRows(db, `SELECT user_costume_uuid, slot_number, ability_id, ability_level, latest_version FROM user_costume_lottery_effect_ability WHERE user_id=?`, uid,
		func(rows *sql.Rows) {
			var e store.UserCostumeLotteryEffectAbilityState
			var key store.CostumeLotteryEffectKey
			rows.Scan(&key.UserCostumeUuid, &key.SlotNumber, &e.AbilityId, &e.AbilityLevel, &e.LatestVersion)
			e.UserCostumeUuid = key.UserCostumeUuid
			e.SlotNumber = key.SlotNumber
			u.CostumeLotteryEffectAbilities[key] = e
		})

	queryRows(db, `SELECT user_costume_uuid, status_calculation_type, hp, attack, vitality, agility, critical_ratio, critical_attack, latest_version FROM user_costume_lottery_effect_status_up WHERE user_id=?`, uid,
		func(rows *sql.Rows) {
			var e store.UserCostumeLotteryEffectStatusUpState
			rows.Scan(&e.UserCostumeUuid, &e.StatusCalculationType, &e.Hp, &e.Attack, &e.Vitality, &e.Agility, &e.CriticalRatio, &e.CriticalAttack, &e.LatestVersion)
			u.CostumeLotteryEffectStatusUps[store.CostumeLotteryEffectStatusUpKey{
				UserCostumeUuid: e.UserCostumeUuid, StatusCalculationType: e.StatusCalculationType,
			}] = e
		})
	...
	queryRows(db, `SELECT character_viewer_field_id, is_notified, notified_datetime, latest_version FROM user_character_viewer_fields WHERE user_id=?`, uid,
		func(rows *sql.Rows) {
			var v store.CharacterViewerFieldState
			var in int
			rows.Scan(&v.CharacterViewerFieldId, &in, &v.NotifiedDatetime, &v.LatestVersion)
			v.IsNotified = in != 0
	...
	queryRows(db, `SELECT pvp_point, attack_win_count, attack_lose_count, defense_win_count,
		defense_lose_count, attack_win_streak, last_finish_day, defense_deck_number,
		...
		max_season_rank, max_season_rank_season_id FROM user_pvp_state WHERE user_id=?`, uid,
		func(rows *sql.Rows) {
			rows.Scan(&u.Pvp.PvpPoint, &u.Pvp.AttackWinCount, &u.Pvp.AttackLoseCount,
				&u.Pvp.DefenseWinCount, &u.Pvp.DefenseLoseCount, &u.Pvp.AttackWinStreak, &u.Pvp.LastFinishDay,
				&u.Pvp.DefenseDeckNumber, &u.Pvp.WeeklyRewardVersion, &u.Pvp.PendingWeeklyRewardGroupId,
				&u.Pvp.PendingWeeklyRewardPoint, &u.Pvp.PendingWeeklyRewardSeasonId, &u.Pvp.LastAutoProgressDay,
				&u.Pvp.MatchingRefreshCount, &u.Pvp.MatchingRefreshDay, &u.Pvp.SeasonIndex, &u.Pvp.LatestVersion,
				&u.Pvp.PvpDeckBackup, &u.Pvp.MaxSeasonRank, &u.Pvp.MaxSeasonRankSeasonId)
		})
