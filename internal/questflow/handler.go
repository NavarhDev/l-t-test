...
type QuestHandler struct {
	*masterdata.QuestCatalog
	...
	MissionCatalog                 *masterdata.MissionCatalog
	MaterialCatalog                *masterdata.MaterialCatalog
	ConsumableItemCatalog          *masterdata.ConsumableItemCatalog
	// FirstClearUnmultipliedMaterialIds contains material IDs whose authored
	// first-clear quantity must be kept exactly (for example ascension books).
	FirstClearUnmultipliedMaterialIds map[int32]bool
	ImportantItems                    *masterdata.ImportantItemCatalog
}

func NewQuestHandler(catalog *masterdata.QuestCatalog, config *masterdata.GameConfig, sideStory *masterdata.SideStoryCatalog, campaigns *campaign.Catalog, characterRebirth *masterdata.CharacterRebirthCatalog, materialCatalog *masterdata.MaterialCatalog, consumableItemCatalog *masterdata.ConsumableItemCatalog) *QuestHandler {
	...
	return &QuestHandler{
		...
		ConsumableItemCatalog:          consumableItemCatalog,
	}
}
...
func BuildGranter(catalog *masterdata.QuestCatalog, config *masterdata.GameConfig) *store.PossessionGranter {
	...
	return &store.PossessionGranter{
		...
		PartsSetGroupIdsByGroupId:            catalog.SetGroupIdsByGroupId,
