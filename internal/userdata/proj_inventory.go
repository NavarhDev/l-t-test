...
func init() {
	...
	register("IUserCageOrnamentAccess", func(user store.UserState) string {
		s, _ := utils.EncodeJSONMaps(sortedCageOrnamentAccessRecords(user)...)
		return s
	})
	...
	register("IUserCostumeLotteryEffectAbility", func(user store.UserState) string {
		s, _ := utils.EncodeJSONMaps(sortedCostumeLotteryEffectAbilityRecords(user)...)
		return s
	})
	register("IUserCostumeLotteryEffectStatusUp", func(user store.UserState) string {
		s, _ := utils.EncodeJSONMaps(sortedCostumeLotteryEffectStatusUpRecords(user)...)
		return s
	})
}
...
func sortedCostumeLevelBonusReleaseStatusRecords(user store.UserState) []map[string]any {
	// Field names are not yet verified against the client schema; returning
	// empty avoids deserialization hangs. Release status is still tracked
	// server-side via RegisterLevelBonusConfirmed.
...
func SortedMaterialRecords(user store.UserState) []map[string]any {
	...
	for _, id := range ids {
		records = append(records, map[string]any{
			...
			"firstAcquisitionDatetime": int64(0),
...
func sortedCageOrnamentAccessRecords(user store.UserState) []map[string]any {
	ids := make([]int, 0, len(user.CageOrnamentAccesses))
	for id := range user.CageOrnamentAccesses {
		ids = append(ids, int(id))
	}
	sort.Ints(ids)
	...
	records := make([]map[string]any, 0, len(ids))
	for _, id := range ids {
		row := user.CageOrnamentAccesses[int32(id)]
		records = append(records, map[string]any{
			"userId":                user.UserId,
			"cageOrnamentId":        row.CageOrnamentId,
			"firstAccessDatetime":   row.FirstAccessDatetime,
			"latestAccessDatetime":  row.LatestAccessDatetime,
			"latestVersion":         row.LatestVersion,
		})
	}
	return records
...
func sortedCostumeLotteryEffectAbilityRecords(user store.UserState) []map[string]any {
	keys := make([]store.CostumeLotteryEffectKey, 0, len(user.CostumeLotteryEffectAbilities))
	...
	records := make([]map[string]any, 0, len(keys))
	for _, k := range keys {
		row := user.CostumeLotteryEffectAbilities[k]
		records = append(records, map[string]any{"userId": user.UserId, "userCostumeUuid": row.UserCostumeUuid, "slotNumber": row.SlotNumber, "abilityId": row.AbilityId, "abilityLevel": row.AbilityLevel, "latestVersion": row.LatestVersion})
	}
	return records
}

func sortedCostumeLotteryEffectStatusUpRecords(user store.UserState) []map[string]any {
	keys := make([]store.CostumeLotteryEffectStatusUpKey, 0, len(user.CostumeLotteryEffectStatusUps))
	...
	records := make([]map[string]any, 0, len(keys))
	for _, k := range keys {
		row := user.CostumeLotteryEffectStatusUps[k]
		records = append(records, map[string]any{"userId": user.UserId, "userCostumeUuid": row.UserCostumeUuid, "statusCalculationType": row.StatusCalculationType, "hp": row.Hp, "attack": row.Attack, "vitality": row.Vitality, "agility": row.Agility, "criticalRatio": row.CriticalRatio, "criticalAttack": row.CriticalAttack, "latestVersion": row.LatestVersion})
	}
	return records
}
