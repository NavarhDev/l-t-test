...
func ChangedTables(before, after *store.UserState) []string {
	...
	if !mapsEqualStruct(before.Missions, after.Missions) {
		add("IUserMission")
		// IUserMissionCompletionProgress and IUserMissionPassPoint are virtual
		// tables whose projectors return "[]" (field names not yet verified
		// against client schema). Do NOT flag them as changed to avoid sending
		// empty diff entries that the client cannot handle.
	}
	...
	if before.Pvp.DefenseDeckNumber != after.Pvp.DefenseDeckNumber {
		add("IUserPvpDefenseDeck")
	}
